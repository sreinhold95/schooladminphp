...
<?php
		$query = $mysqli->query("SELECT * FROM students WHERE (surname='".$search."') OR (middlename LIKE '%".$search."%') OR (classcode LIKE '%".$search."%') OR (givenname LIKE '%".$search."%') OR (moregivenname LIKE'%".$search."%')");
        if ( $query->num_rows ) {
            while ( $get = $query->fetch_assoc() ) {
                echo '<tr>';
                if ( $get[ 'active' ] == 1 ) {
