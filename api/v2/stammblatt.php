...
$auth=false;
//uuid teacher-ID
$query =$mysqli->query("select teacher from user where uuid='".$uuid."' and username='".$username."'");
if($query->num_rows){
    while($quererg=$query->fetch_assoc()){
        $idteacher=$queryerg["teacher"];
        $auth=true;
    }
}
if($auth){
